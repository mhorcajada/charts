...
[![Helm Chart Version](https://img.shields.io/badge/helm--chart-v0.3.3-blue?logo=helm)](https://github.com/mhorcajada/charts/tree/gh-pages)
...
```bash
helm template mhorcajada/storj-gateway --version 0.3.3 -f values-vault.yaml
helm template mhorcajada/storj-gateway --version 0.3.3 -f values-envFrom-Secret.yaml
helm template mhorcajada/storj-gateway --version 0.3.3 -f values-envFrom-configMap.yaml
helm template mhorcajada/storj-gateway --version 0.3.3 -f values-initContainer-configMap.yaml
```

Para comprobar sintaxis e inyección de variables:

```bash
helm install storj-release mhorcajada/storj-gateway --version 0.3.3 -f values-<modo>.yaml --dry-run --debug
```
